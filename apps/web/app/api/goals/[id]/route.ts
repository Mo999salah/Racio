import { savingsGoalActionSchema, savingsGoalPatchSchema } from '@racio/contracts';
import { AuthBoundaryError, getUserPreferences, requireSession } from '@racio/auth';
import { actionSavingsGoal, getGoalProgress, updateSavingsGoal } from '@racio/planning';
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const session = await requireSession(await headers());
    const parsed = savingsGoalPatchSchema.safeParse(await request.json());
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    assertSameOrigin(request);
    const session = await requireSession(await headers());
    const parsed = savingsGoalActionSchema.safeParse(await request.json());
    if (!parsed.success) throw new AuthBoundaryError('VALIDATION', 'Invalid savings goal action.');
    const result = await actionSavingsGoal(
      database.db,
      session.user.id,
      (await params).id,
      parsed.data.action,
